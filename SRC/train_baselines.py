...
CSV = "../data/PRSA_data_2010.1.1-2014.12.31.csv"
...
plt.ylabel("Actual")
plt.tight_layout()
plt.savefig(FIGS_DIR / "plot3_confusion_matrix.png")
plt.show()
...
plt.tight_layout()
plt.savefig(FIGS_DIR / "plot4_residuals_vs_predicted.png")
plt.show()

pd.DataFrame(clf_metrics, columns=['Model','Accuracy','F1']).to_csv(TABLES_DIR / "table1_classification_metrics.csv", index=False)
pd.DataFrame(reg_metrics, columns=['Model','MAE','RMSE']).to_csv(TABLES_DIR / "table2_regression_metrics.csv", index=False)
